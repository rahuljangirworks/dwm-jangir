...
read -r -a fields <<<"$rest"
trap '' TERM
printf '%s:%s\n' "$$" "${fields[19]}" >"$TEST_WATCH_MONITOR_ID"
...
for identity in "${watch_test_identities[@]}"; do
	if identity_is_live "$identity"; then
		printf 'display watch process survived direct owner exit: %s\n' "$identity" >&2
		exit 1
	fi
done
...
wait "$dead_owner_pid"
if env "${settings_env[@]}" TEST_WATCH_MONITOR_ID="$work/dead-owner-monitor.id" \
	TEST_SLEEP_BIN="$sleep_bin" "$BASH_BIN" "$SETTINGS_HELPER" watch \
	"$dead_owner_pid" "$dead_owner_start" >"$work/dead-owner.out" 2>"$work/dead-owner.err"; then
	printf 'display watch accepted an owner that exited before helper startup\n' >&2
	exit 1
fi
grep -Fq 'display watch owner is unavailable' "$work/dead-owner.err"
[[ ! -e $work/dead-owner-monitor.id ]]

test_owner_start=$(awk '{ line = $0; sub(/^.*\) /, "", line); split(line, fields, " "); print fields[20] }' \
	"/proc/$$/stat")
if env "${settings_env[@]}" TEST_WATCH_MONITOR_EXIT=1 \
	"$BASH_BIN" "$SETTINGS_HELPER" watch "$$" "$test_owner_start" \
	>"$work/monitor-exit.out" 2>"$work/monitor-exit.err"; then
	printf 'display watch accepted an event monitor that exited unexpectedly\n' >&2
	exit 1
fi
