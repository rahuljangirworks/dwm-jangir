...
#
# Updates: alacritty · kitty · dwmterm · GTK · Qt
...
if [[ -z "$THEME_RUNTIME_BASE" ]]; then
	THEME_RUNTIME_BASE=/tmp/dwm-jangir-$UID
...
# ── Locate themes.toml ────────────────────────────────────────────────────────
THEMES_FILE="${DWM_APPEARANCE_THEMES_FILE:-${XDG_CONFIG_HOME:-$HOME/.config}/dwm-jangir/themes.toml}"
MANAGED_THEMES_FILE="${DWM_APPEARANCE_MANAGED_THEMES_FILE:-${XDG_DATA_HOME:-$HOME/.local/share}/dwm-jangir/config/themes.toml}"
...
if [[ $RUNTIME_ONLY_EXPLICIT == false ]]; then
	...
	if [[ $AUTOMATIC_APPLY == 1 ]]; then
		THEME_SUPPRESS_FILE=$THEME_STATE_HOME/dwm-jangir/appearance/integration-suppress
	...
	THEME_TRANSACTION_FILE=$THEME_STATE_HOME/dwm-jangir/appearance/integration-transaction
...
PERSONALIZATION_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/dwm-jangir/personalization.conf"
...
# Change the theme in ~/.config/dwm-jangir/themes.toml instead.
...
CURSOR_XRESOURCES="${XDG_CONFIG_HOME:-$HOME/.config}/dwm-jangir/cursor.Xresources"
if [[ $RUNTIME_ONLY == 0 && $LIVE_ONLY == 0 ]]; then
...
THEME_ENV_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/dwm-jangir/theme-env.sh"
if [[ $RUNTIME_ONLY == 0 && $LIVE_ONLY == 0 ]]; then
