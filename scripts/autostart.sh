#!/bin/sh
# dwm-jangir autostart — single unified autostart script
...
start_detached_display_command_once() {
	...
	if [ -n "${XDG_RUNTIME_DIR:-}" ] && [ -d "$XDG_RUNTIME_DIR" ] &&
		...
		display_lock_dir=$XDG_RUNTIME_DIR/dwm-jangir
...
THEME_ENV="${XDG_CONFIG_HOME:-$HOME/.config}/dwm-jangir/theme-env.sh"
...
if [ -n "$xsettings_helper" ]; then
	timeout --signal=TERM --kill-after=1 8 \
		...
		printf '%s\n' 'dwm-jangir: managed X11 text scaling is unavailable' >&2
fi
...
if [ -f "$QUICKSHELL_CONFIG" ]; then
	...
	*/*)
	...
	if [ -n "$quickshell_check" ] && "$quickshell_check"; then
		...
		printf '%s\n' 'dwm-jangir: compatible Quickshell 0.3.0 or Fedora 44 snapshot is required' >&2
	fi
