...
| `sync/2026-09-01` | merged as `4153460` | Current fork work rebased onto `upstream/main` at `70e6e43`; validated in the Fedora VM and merged into `dev`. |
...
| D-008 | `lightdm/`, installer, package map, tests, and docs | Offer the VM1 Solar LightDM design as an optional image-free greeter without replacing upstream Slick Greeter. | Keep `stock` as the default. Build Solar only when explicitly selected, pin it to Slick Greeter 2.2.6, preserve an existing Solar selection on a no-flag re-run, and retain its files when switching back to stock. Do not restore the historical wallpaper bundle or make network weather a login dependency. | Run LightDM staging tests, installer option checks, RPM build/package checks, and a dedicated Fedora LightDM VM login validation. |

Only D-001 through D-008 are approved. A new difference must be assigned a new
...
| 2026-09-10 | Rebased onto upstream `40cbdc8` on `sync/2026-09-10`, then merged it into `dev` as `fa8d624`. Retained D-001 through D-007, renamed newly imported runtime paths to `dwm-jangir`, and adopted upstream Slick Greeter by removing the fork-specific overlay, RPM, and wallpaper assets. | Merged locally; not pushed. |
