...
if (($# > 1)); then
    printf 'usage: %s [--print-rpm] [source archive]\n' "${BASH_SOURCE[0]}" >&2
    exit 2
fi
...
if [[ $print_rpm == true ]]; then
    rpmbuild -ba "$topdir/SPECS/dwm-jangir-slick-greeter.spec" >&2
    mapfile -t packages < <(find "$topdir/RPMS" -type f \
        -name "dwm-jangir-slick-greeter-${version}-*.rpm" -print)
    if ((${#packages[@]} != 1)); then
        printf 'Expected exactly one built Solar greeter RPM, found %d.\n' \
            "${#packages[@]}" >&2
        exit 1
    fi
    printf '%s\n' "${packages[0]}"
